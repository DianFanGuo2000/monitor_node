...
int sync_communication_info(const char* listened_interface, const char* center_interface_name)
{
	...
	{
		// non_center_node reponsible for sending communication info to center_node
...
void deal_with_mnt(const char* linked_node,const char* listened_interface, const char* msg) {  
	...
	if(strcmp(msg, expect_msg) == 0)
    {
		...
		if (current_round > round_array[ind]) { 
			...
			/*
		    double ratio = 1 - (double)cnt_array[ind]/ PACKAGES_NUM_ONE_TIME;  
		    snprintf(result, RESULT_STRING_SIZE, "%.2f", ratio); 
			printf("the interface \"%s\" got an error ratio value with %s ( tx:%d rx:%d)\n",listened_interface,result,PACKAGES_NUM_ONE_TIME,cnt_array[ind]); 
			free(result);	*/

			update_communication_info_array(linked_node,listened_interface,time(NULL),PACKAGES_NUM_ONE_TIME,cnt_array[ind]);
			printf("\n");
...
{  
	...
	if (current_round > round_array[ind]) { 
		...
		delay.tv_sec = 0;  
		delay.tv_nsec = WAITING_TIME_FOR_SENDING_AFTER_ONE_ROUND_CHANGING_POINT;
		nanosleep(&delay, NULL);
		
		delay.tv_sec = 0; 
		delay.tv_nsec = SENDING_TIME_SPEC;
