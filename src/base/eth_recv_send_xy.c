...
#include "eth_recv_send_xy.h"



// 在程序初始化时创建锁  
void initialize_eth_lock_xy() {  
    pthread_mutex_init(&eth_lock_xy, NULL);  
}  
  
// 在程序结束时销毁锁  
void destroy_eth_lock_xy() {  
    pthread_mutex_destroy(&eth_lock_xy);  
}
...
int stringToMacAddress_xy(const char* macStr, unsigned char* macAddr) {  
...
int send_packet_xy(int sockfd, const struct sockaddr_ll* sock_addr_value_addr, const char *message, const char *ether_shost, const char *ether_dhost)  
{
	...
	stringToMacAddress_xy(ether_shost,srcmac);
	stringToMacAddress_xy(ether_dhost,dstmac);
