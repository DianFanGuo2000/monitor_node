...
int receive_message(const char *linked_node,const char *source_interface,Dealer deal,long max_waiting_time)    
{
	...
	{
		return _ERROR; /* 单纯sending或closed，那么不做任何接收 */
	}
    ...
    if (strcmp(base_receive_func, "receive_packet") == 0) {   
		...
		strncpy(data.msg, (const char*)TEMP_MSG, MAX_MSG_LEN); // 不直接拿data.msg作为形参，防止其随着原函数声明周期结束而被析构
	...
	if (strcmp(base_receive_func, "receive_packet_can_gpu") == 0) { 
		...
		strncpy(data.msg, (const char*)TEMP_MSG, MAX_CAN_DATA_LENGTH); // 不直接拿data.msg作为形参，防止其随着原函数声明周期结束而被析构
